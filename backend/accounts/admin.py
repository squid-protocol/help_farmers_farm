...
class FormSignatureInline(admin.TabularInline):
    ...
    readonly_fields = (
        'form', 'digital_signature', 'signed_at', 
        'is_guardian_signature', 'signer_ip_address', 'document_hash'
    )
...
class FormSignatureAdmin(admin.ModelAdmin):
    ...
    """
    list_display = ('user', 'get_farm_name', 'form', 'digital_signature', 'signed_at', 'has_pdf')
    list_filter = ('form__farm', 'signed_at', 'is_guardian_signature')
    search_fields = ('user__username', 'user__email', 'digital_signature', 'document_hash', 'signer_ip_address')
    
    # Lock down ALL fields. This is an audit log, not an editable table.
    readonly_fields = (
        'user', 'form', 'digital_signature', 'signed_at', 
        'is_guardian_signature', 'guardian_relationship', 
        'signer_ip_address', 'document_hash', 'pdf_receipt'
    )

    fieldsets = (
        ('Signer Information', {
            'fields': ('user', 'digital_signature', 'signed_at')
        }),
        ('Document Details', {
            'fields': ('form',)
        }),
