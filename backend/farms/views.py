...
def farm_impact_view(request):
    ...
    crops = (
        Crop.objects.filter(farm=farm, is_active=True)
        .exclude(crop_name__iexact="General / Deleted")
        .order_by("crop_name")
    )
...
def farm_search_view(request):
    ...
    for f in farms:
        if f.latitude and f.longitude:
            # SECURITY: Fuzz the coordinates before sending to the browser
            ...
            map_data.append(
                {
                    "id": f.id,
                    "name": f.name,
                    "lat": f.latitude + jitter_lat,
                    "lng": f.longitude + jitter_lng,
