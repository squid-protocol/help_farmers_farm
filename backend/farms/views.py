...
def farm_impact_view(request):
    ...
    aggregated_data = LogEntry.objects.filter(farm=farm).values(
        'crop__crop_name', 'activity'
    ).annotate(
...
# 3. Define your colors (Semantic, modern SaaS palette)
    ...
    for act_code, act_label in activity_labels.items():
        ...
        for crop in crops:
            ...
            hours = next((float(item['total_hours']) for item in aggregated_data 
                          if item['crop__crop_name'] == crop and item['activity'] == act_code), 0)
