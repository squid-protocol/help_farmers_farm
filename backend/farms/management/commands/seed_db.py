...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        crops_a = [
            ...
            for name in ["Heirloom Tomatoes", "Bell Peppers", "Sweet Onions", "Garlic", "Carrots"]
        ]
        crops_b = [
        ...
        edge_cases = [
            {"username": "admin", "email": "admin@system.local", "role": "account_manager", "is_super": True},
            {"username": "manager_a", "email": "manager_a@caledonia.local", "role": "farm_manager"},
            {"username": "manager_b", "email": "manager_b@alto.local", "role": "farm_manager"},
            {"username": "manager_c", "email": "manager_c@rusty.local", "role": "farm_manager"},
            {"username": "veteran_vol", "email": "rosalyn@vol.local", "role": "volunteer", "legacy": 5},
            {"username": "rookie_vol", "email": "lola@vol.local", "role": "volunteer", "legacy": 0},
            {"username": "lone_wolf", "email": "loner@vol.local", "role": "volunteer"},
            {"username": "legacy_ghost", "email": "friend@vol.local", "role": "friend", "legacy": 10},
        ]
        ...
        normal_names = [
            ("Elias", "Stone"), ("Clara", "Vance"), ("Marcus", "Tate"),
            ("Sylvia", "Pond"), ("Julian", "Finch"), ("Naomi", "Rivers"),
            ("Declan", "Cross"), ("Vera", "Hull"), ("Silas", "Mercer"), 
            ("Iris", "Locke")
        ]
        ...
        for first, last in normal_names:
            ...
            user, created = User.objects.get_or_create(
                ...
                defaults={
                    ...
                    "farming_motto": random.choice([
                        "Soil over systems.", "Just happy to be outside.", 
                        "Will weed for tomatoes.", "Learning the ropes.", ""
                    ])
                }
            )
            if created:
            ...
            for farm, crops, tier in farms_joined:
                FarmMembership.objects.get_or_create(
                    user=user, farm=farm, is_approved=True, agreed_to_waiver=True, work_commitment=tier
                )
        ...
        self.stdout.write(self.style.SUCCESS("✅ Matrix seeded successfully. All personas and noise data are live."))