...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        farms_to_update = Farm.objects.filter(address_line1__isnull=False, latitude__isnull=True).exclude(
            address_line1=""
        )
        ...
        self.stdout.write(self.style.WARNING(f"Found {total} farms to geocode. Starting..."))
        ...
        for farm in farms_to_update:
            try:
                ...
                if location:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"✅ Success: {farm.name} -> [{location.latitude}, {location.longitude}]")
                    )
                    success_count += 1
                else:
                    self.stdout.write(
                        self.style.ERROR(f"❌ Failed completely: {farm.name} (Address not found in database)")
                    )
                    fail_count += 1

            except (GeocoderTimedOut, GeocoderServiceError) as e:
                self.stdout.write(self.style.ERROR(f"⚠️ API Error for {farm.name}: {e}"))
                fail_count += 1
